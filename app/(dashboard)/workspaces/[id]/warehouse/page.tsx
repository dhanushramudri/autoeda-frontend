...
function ColumnRow({ col, onClick }: { col: CatalogColumn; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className="w-full flex items-center gap-2 px-3 py-[3px] hover:bg-muted transition text-left group"
      title="Click to insert reference"
    >
      <Columns className="w-3 h-3 text-muted-foreground/60 flex-shrink-0" />
      <span className="font-mono text-[11px] text-muted-foreground flex-1 truncate">{col.name}</span>
    ...
    <div>
      ...
      >
        ...
        <div className="flex-1 min-w-0 text-left">
          ...
          {slug !== label && slug && (
            <span className="font-mono text-[9px] text-muted-foreground truncate block">{slug}</span>
          )}
        </div>
      ...
      {open && (
        <div className="ml-3 border-l border-border pl-1">
          {isLoadingCols ? (
            ...
            <p className="px-3 py-1 text-[10px] text-muted-foreground">No column info</p>
          ) : (
            columns.map((col) => (
  ...
  return (
    <div className="border-b border-border last:border-b-0">
      ...
      >
        ...
        <span className="text-xs font-semibold text-foreground flex-1 text-left truncate">{groupLabel}</span>
        <span className="text-[9px] text-muted-foreground flex-shrink-0">{sections.length} sources · {totalItems}</span>
        {collapsed ? <ChevronRight className="w-3.5 h-3.5 text-muted-foreground flex-shrink-0" /> : <ChevronDown className="w-3.5 h-3.5 text-muted-foreground flex-shrink-0" />}
      </button>
...
export default function WarehousePage() {
  ...
  return (
    <div className="flex h-[calc(100vh-64px)] overflow-hidden bg-muted">
      ...
      {/* —— Catalog panel ——————————————————————————————————————————————————————————————— */}
      <div
        className="border-r border-border bg-card flex flex-col flex-shrink-0 transition-all duration-200"
        style={{ width: showCatalog ? 280 : 0, minWidth: showCatalog ? 280 : 0 }}
      >
        {showCatalog && (
          <>
            {/* Header */}
            <div className="flex items-center gap-2 px-3 py-2.5 border-b border-border bg-muted flex-shrink-0">
              <Database className="w-4 h-4 text-brand" />
              <span className="text-xs font-bold text-foreground flex-1">Warehouse Catalog</span>
              <button
            ...
            <div className="border-t border-border p-2 flex-shrink-0">
              <button
                ...
                className="w-full flex items-center justify-center gap-1.5 py-1.5 text-[10px] text-muted-foreground hover:text-muted-foreground hover:bg-muted rounded transition"
              >
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="flex items-center gap-2 px-4 py-2 border-b border-border bg-card flex-shrink-0">
          {!showCatalog && (
            <button
              ...
              className="p-1.5 rounded hover:bg-muted text-muted-foreground mr-1"
              title="Show catalog"
            >
              <Code2 className="w-4 h-4" />
            </button>
          )}
          <span className="text-sm font-bold text-foreground">Warehouse</span>
        ...
        {/* Monaco */}
        <div className="border-b border-border flex-shrink-0" style={{ height: 220 }}>
          <MonacoEditor
            height="220px"
            language="sql"
            value={sql}
        ...
        <div className="flex-1 flex flex-col min-h-0">
          ...
          <div className="flex items-center border-b border-border bg-card px-3 flex-shrink-0">
            ...
            {result && !result.error && result.elapsed_ms > 0 && (
              <span className="text-[10px] text-muted-foreground">{result.elapsed_ms}ms</span>
            )}
            {result && !result.error && result.columns.length > 0 && (
              <button onClick={copyResult} className="ml-2 p-1.5 text-muted-foreground hover:text-foreground transition" title="Copy as TSV">
          ...
          <div className="flex-1 min-h-0">
            ...
            {!loading && activeTab === "results" && (
              <>
                ...
                {result && !result.error && result.columns.length === 0 && (
                  <div className="flex items-center justify-center h-full text-muted-foreground text-sm">Query returned no rows</div>
                )}
              </>
            )}
