...
function renderTopValues(v: ColumnProfile["top_values"]) {
  if (!v || v.length === 0) return <span className="text-muted-foreground/60">—</span>;
  return (
    <div className="flex flex-wrap gap-1">
      {v.slice(0, 3).map((item, i) => (
        <span key={`${item.value}-${i}`} className="px-1.5 py-0.5 bg-muted rounded text-[10px] text-muted-foreground max-w-[100px] truncate">
...
function OverviewTab({ data, allMeta, onSelectCol, onToggleTag, onExport }: OverviewTabProps) {
  ...
  function SortIcon({ colKey }: { colKey: SortKey }) {
    if (sortKey !== colKey) return <ChevronDown className="w-3 h-3 text-muted-foreground/60 ml-0.5 flex-shrink-0" />;
    return sortDir === "asc"
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div className="bg-card border border-border rounded-xl p-4">
          ...
          <p className="text-2xl font-bold text-foreground">{data.total_columns}</p>
        </div>
        <div className="bg-card border border-border rounded-xl p-4">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <div className="relative">
          ...
          <input
            ...
            className="pl-8 pr-3 py-2 text-sm border border-border rounded-lg w-56 focus:outline-none focus:ring-2 focus:ring-brand focus:border-brand transition"
          />
          {search && (
        ...
        <span className="text-xs text-muted-foreground">{columns.length} of {data.total_columns} columns</span>
        <button
          onClick={onExport}
          className="ml-auto flex items-center gap-1.5 px-3 py-2 border border-border rounded-lg text-sm text-muted-foreground hover:bg-muted transition"
        >
      ...
      <div className="bg-card rounded-xl border border-border overflow-x-auto">
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border bg-muted/80">
              <th className="px-4 py-2.5 text-left text-xs font-semibold text-muted-foreground whitespace-nowrap">Column</th>
              <th className="px-4 py-2.5 text-left text-xs font-semibold text-muted-foreground whitespace-nowrap">Type</th>
              {SORT_HEADERS.map(({ label, key }) => (
                <th
                  key={key}
                  className="px-4 py-2.5 text-left text-xs font-semibold text-muted-foreground whitespace-nowrap cursor-pointer select-none hover:text-foreground transition-colors"
                  onClick={() => toggleSort(key)}
                >
                  <span className="inline-flex items-center">{label}<SortIcon colKey={key} /></span>
                </th>
              ))}
              <th className="px-4 py-2.5 text-left text-xs font-semibold text-muted-foreground whitespace-nowrap">Top Values</th>
              <th className="px-4 py-2.5 text-left text-xs font-semibold text-muted-foreground whitespace-nowrap">Tags</th>
            </tr>
          </thead>
          <tbody>
            {columns.map((col) => {
              ...
              return (
                <tr key={col.name} className="border-b border-border hover:bg-blue-50/30 transition-colors group">
                  ...
                  <td className="px-4 py-2.5">
                    <span className={cn("px-2 py-0.5 rounded text-[10px] font-semibold", TYPE_COLOR[col.semantic_type] ?? "bg-muted text-muted-foreground")}>
                      {col.semantic_type}
                    </span>
                  </td>
                  <td className="px-4 py-2.5"><MiniBar pct={col.missing_pct} /></td>
                  <td className="px-4 py-2.5 text-xs text-muted-foreground tabular-nums">{col.unique_count?.toLocaleString()}</td>
                  <td className="px-4 py-2.5 font-mono text-xs text-muted-foreground">{col.mean != null ? col.mean.toFixed(3) : "—"}</td>
                  ...
                  <td className="px-4 py-2.5 font-mono text-xs text-muted-foreground">{col.min != null ? String(col.min) : "—"}</td>
                  <td className="px-4 py-2.5 font-mono text-xs text-muted-foreground">{col.max != null ? String(col.max) : "—"}</td>
                  ...
                  <td className="px-4 py-2.5">
                    <div className="relative flex flex-wrap gap-1 items-center">
                      {tags.map((tag) => (
                        <span
                          key={tag}
                          className={cn("text-[10px] px-1.5 py-0.5 rounded-full font-medium inline-flex items-center gap-0.5", TAG_COLORS[tag] ?? "bg-muted text-muted-foreground")}
                        >
                          {tag}
                          <button
                      ...
                      <button
                        ...
                        className="text-[10px] text-muted-foreground hover:text-brand transition flex items-center gap-0.5"
                      >
                        <Tag className="w-2.5 h-2.5" />
                      </button>
                      {editingTagCol === col.name && (
                        <div className="absolute z-30 top-6 left-0 bg-card border border-border rounded-xl shadow-xl p-2 flex gap-1 flex-wrap min-w-[170px]">
                          {QUICK_TAGS.map((t) => (
                            <button
                              key={t}
        ...
        {columns.length === 0 && (
          <div className="py-10 text-center text-sm text-muted-foreground">No columns match your search.</div>
        )}
      </div>
    </div>
  );
      ...
      {missingCols.length === 0 ? (
        ...
        <>
          <div className="bg-card rounded-xl border border-border p-6">
            <h2 className="text-sm font-semibold text-foreground mb-4">Missing by Column</h2>
  ...
  if (numericCols.length === 0) {
    return (
      <div className="bg-card rounded-xl border border-border p-12 text-center">
        <BarChart2 className="w-10 h-10 mx-auto mb-3 text-muted-foreground/60" />
        <p className="text-sm text-muted-foreground">No numeric columns in this dataset.</p>
      </div>
    );
  }
      ...
      <div className="w-52 flex-shrink-0 space-y-2">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-7 pr-3 py-1.5 text-xs border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-brand transition"
          />
        </div>
        ...
        <div className="space-y-0.5 max-h-[65vh] overflow-y-auto scrollbar-hide pr-1">
          ...
          {filtered.length === 0 && (
            <p className="text-xs text-muted-foreground px-2 py-3">No columns match.</p>
          )}
        </div>
      </div>

      {/* Chart panel */}
      <div className="flex-1 bg-card rounded-xl border border-border p-6 min-h-[420px]">
        {!activeCol ? (
      ...
      <div className="flex flex-wrap gap-3 items-center">
        ...
        {method !== "isolation_forest" && numericCols.length > 0 && (
          <select
            ...
            className="px-3 py-1.5 text-xs border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-brand bg-card"
          >
            {numericCols.map((c) => (
          ...
          {data.total_outliers === 0 ? (
            ...
            <div className="bg-card rounded-xl border border-border overflow-hidden">
              <div className="px-5 py-3 border-b border-border">
                <h3 className="text-sm font-semibold text-foreground">Outliers by Column</h3>
              </div>
              <DataTable
                columns={[
                  {
                  ...
                  },
                  {
                    ...
                    align: "right" as const,
                    render: (v) => v != null
                      ? <span className="font-mono text-xs text-muted-foreground">{Number(v).toFixed(3)}</span>
    ...
    <>
      ...
      <div className="p-6 max-w-full mx-auto">
        ...
        <div className="mt-4 mb-5">
          <h1 className="text-xl font-bold text-foreground">Data Profile</h1>
        ...
        {/* Tab navigation */}
        <div className="flex border-b border-border mb-6 -mx-0 gap-0">
