...
function SearchColumnsSummary({ result }: { result: Record<string, unknown> }) {
  ...
  if (!matches.length) return <p className="text-xs text-gray-400 italic px-1">No matching columns found.</p>;
  return (
...
export function HypothesisToolResultPreview({ call }: { call: ScoutToolCall }) {
  ...
  switch (call.tool) {
    case "get_missing":
      return <div className="rounded-xl border border-gray-100 bg-white p-3"><MissingHeatmap data={r as any} /></div>; // eslint-disable-line @typescript-eslint/no-explicit-any
    case "get_correlations":
      return <div className="rounded-xl border border-gray-100 bg-white p-3 overflow-x-auto"><CorrelationHeatmap data={r as any} /></div>; // eslint-disable-line @typescript-eslint/no-explicit-any
    case "get_outliers":
      return <OutlierSummary result={r} />;
    case "get_quality_score":
      return <div className="rounded-xl border border-gray-100 bg-white p-3"><QualityGauge data={r as any} /></div>; // eslint-disable-line @typescript-eslint/no-explicit-any
    case "run_sql":
    ...
    case "get_distribution": {
      ...
      return <div className="rounded-xl border border-gray-100 bg-white p-3"><DistributionChart data={r as any} column={column} /></div>; // eslint-disable-line @typescript-eslint/no-explicit-any
    }
    case "search_columns":
